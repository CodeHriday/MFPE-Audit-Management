...
class AuditSeverityControllerTests {
	...
	public void testAuditSeverityStatus() {
		...
		ResponseEntity<List<AuditBenchmark>> responseEntity = new ResponseEntity<List<AuditBenchmark>>(benchmarkList,HttpStatus.OK);
		when(auditBenchmarkFeign.getAuditBenchmark("jwt")).thenReturn(responseEntity);
